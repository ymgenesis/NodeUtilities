...
@invocation(
    ...
    title="Remove Transparencies",
    tags=["image", "remove", "transparencies", "crop"],
    category="image",
    version="1.0.0",
)
class RemoveTransparenciesInvocation(BaseInvocation):
    ...
    transparency_threshold: float = InputField(
        default=0.5, description="Transparency threshold pixels meet to be removed. 0 = transparent, 1 = opaque."
    )
    crop: bool = InputField(default=False, description="Whether to crop to remaining pixels. H&W both a multiple of 8.")
    border: int = InputField(
        default=0,
        description="If cropping, the transparent border (px) to draw around the cropped area. >0 & multiple of 8.",
    )
    rectangle_mask: bool = InputField(
        default=False,
        description="Whether the mask equals the entire bounding box dimensions of the subject(s). Off when crop is on. Off will pass a form-fitting mask. On provides a larger mask for painting to work with.",
    )
