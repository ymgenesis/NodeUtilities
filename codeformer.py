...
from basicsr.utils import get_root_logger, img2tensor, tensor2img
...
def inchannormalize(in_channels):
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels=None):
        ...
        self.norm1 = inchannormalize(in_channels)
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.norm2 = inchannormalize(out_channels)
...
class AttnBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.norm = inchannormalize(in_channels)
...
class Encoder(nn.Module):
    def __init__(self, in_channels, nf, emb_dim, ch_mult, num_res_blocks, resolution, attn_resolutions):
        ...
        blocks.append(inchannormalize(block_in_ch))
...
@invocation(
    "codeformer_face_restoration",
    title="CodeFormer",
    tags=["image", "face", "restoration", "codeformer"],
    category="image",
    version="1.0.0",
)
class CodeFormerInvocation(BaseInvocation):
    ...
    def invoke(self, context: InvocationContext) -> ImageOutput:
        ...
        for idx, cropped_face in enumerate(face_helper.cropped_faces):
            ...
            try:
                with torch.no_grad():
                    output = cf(cropped_face_t, w=self.fidelity, adain=True)[0]
